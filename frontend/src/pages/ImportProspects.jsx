...
export default function ImportProspects() {
  ...
  const handleImport = async () => {
    setLoading(true);
    try {
      await api.post('/import-prospects/', { prospects });
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 sm:p-12 text-center transition-colors cursor-pointer ${
            dragActive ? 'border-blue-500 bg-blue-50' : 'border-blue-200 bg-slate-50/50 hover:bg-slate-50'
          }`}
        >
      ...
      {/* Tableau Aperçu */}
      <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm overflow-hidden">
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="py-4 pl-4 text-right sm:text-left hidden sm:table-cell">
                    <button
                      ...
                      className="px-4 py-1.5 border border-slate-200 rounded-full text-xs font-semibold text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer"
                    >
                      {item.action}
                    </button>
                  </td>
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm">
        <button
          ...
          disabled={loading}
          className="w-full bg-[#f59e0b] hover:bg-[#d97706] active:scale-[0.99] text-white font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-2 shadow-sm transition-all disabled:opacity-70 cursor-pointer text-sm sm:text-base"
        >
